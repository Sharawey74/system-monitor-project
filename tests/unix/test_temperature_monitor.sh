...
echo "Testing temperature_monitor.sh..."
...
if [ $exit_code -ne 0 ]; then
    echo "[FAIL] temperature_monitor.sh exited with code $exit_code"
    exit 1
fi
...
# Check for required fields (either temperature data or status)
if ! echo "$output" | grep -q '"temperature"'; then
    echo "[FAIL] Missing 'temperature' field"
    exit 1
fi

echo "[PASS] temperature_monitor.sh"
exit 0
